...
MODEL_DIR = BASE_DIR / "app_model_colab (1)"   # ← Path to XGBoost models
...
def load_models():
    if not MODEL_DIR.exists():
        return {"loaded": False,
                "error": f"Folder 'app_model_colab (1)' not found at {MODEL_DIR}"}
    try:
        ...
        return {
            "loaded":        True,
            "model_full":    joblib.load(MODEL_DIR / "xgb_full_model.pkl"),
            "model_behav":   joblib.load(MODEL_DIR / "xgb_behav_model.pkl"),
...
def get_explainer(mode="full"):
    if not models.get("loaded"):
        return None
    model = models["model_full"] if mode == "full" else models["model_behav"]
...
def build_features(raw: dict, feat_maxes: dict) -> dict:
    ...
    if "gender" in d and isinstance(d["gender"], str):
        d["gender"] = d["gender"].strip().capitalize()
    return d
...
def predict_student(raw_data: dict, mode: str = "full") -> dict:
    ...
    input_df = pd.DataFrame([data], columns=feats)
    for col in input_df.columns:
        if input_df[col].dtype == object or col == "gender":
            try:
                input_df[col] = input_df[col].astype(float)
            except ValueError:
                input_df[col] = input_df[col].astype('category')
        elif input_df[col].dtype.name != 'category':
            input_df[col] = input_df[col].astype(float)
...
/* 🔴 WIDGET OVERRIDES: EXPLICITLY PREVENT STREAMLIT DARK MODE BUGS 🔴 */
...
if not models.get("loaded"):
    st.error(
        ...
        f"Make sure `app_model_colab (1)` sits next to `app.py` and contains "
...
if st.session_state.mode == "student":
    ...
    # Student Profile & Study Habits
    st.markdown('<div class="fg"><div class="fg-title">Student Profile & Habits *</div>', unsafe_allow_html=True)
    ...
    with h1:
        ...
        age      = st.number_input("Age",
                                   ...
                                   value=20, step=1, key="s_age",
                                   help="Your age.")
        prev_gpa = st.number_input("Previous Semester GPA",
                                   min_value=0.0, max_value=4.0,
                                   value=3.0, step=0.1, key="s_gpa",
                                   help="GPA from the previous semester (e.g., 0.0 to 4.0).")
    with h2:
        attend   = st.number_input("Attendance percentage (%)",
                                   min_value=0.0, max_value=100.0,
                                   value=75.0, step=1.0, key="s_attend",
                                   help="Percentage of classes attended.")
        gender   = st.selectbox("Gender", options=["Male", "Female"], key="s_gender",
                                help="Your gender.")
    with h3:
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    <div class="info-box">
      ...
      <div class="info-box-body">
        <strong>student_id, age, gender, previous_semester_gpa, study_hours_per_day, attendance_percentage,
    ...
    if st.button("Analyse Class →", key="btn_lec"):
        if uploaded is None:
            ...
            required = ["age", "gender", "previous_semester_gpa", "study_hours_per_day", "attendance_percentage",
            ...
            with st.spinner(f"Running model on {len(df_up):,} students…"):
                ...
                for _, row in df_up.iterrows():
                    # Preserve raw strings for gender, parse everything else to float
                    raw = {}
                    for c in required:
                        if c == "gender":
                            raw[c] = str(row[c]).strip().capitalize()
                        else:
                            raw[c] = float(row[c])
                            
                    data = build_features(raw, models["feat_maxes"])
                    
                    # Safely handle mixed data types for XGBoost
                    inp = pd.DataFrame([data], columns=models["all_features"])
                    for col in inp.columns:
                        if inp[col].dtype == object or col == "gender":
                            try:
                                inp[col] = inp[col].astype(float)
                            except ValueError:
                                inp[col] = inp[col].astype('category')
                        elif inp[col].dtype.name != 'category':
                            inp[col] = inp[col].astype(float)
                    
                    pred = int(models["model_full"].predict(inp)[0])
                    band = models["int_to_band"][pred]
                    prob = models["model_full"].predict_proba(inp)[0]
    ...
    if st.session_state.lec_results is not None:
        ...
        for sid in sample_ids:
            ...
            for c in required:
                if c == "gender":
                    raw[c] = str(r[c]).strip().capitalize()
                else:
                    raw[c] = float(r[c])
...
<div class="footer">
  ...
  <div class="footer-right">
    <span class="fbadge">XGBOOST ENSEMBLE</span>
